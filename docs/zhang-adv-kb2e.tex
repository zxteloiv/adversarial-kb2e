\documentclass[twocolumn,a4paper,preprint,10pt,3p]{elsarticle}
...
\begin{algorithm*}[htb]
    \DontPrintSemicolon
    \KwIn{Pretrained generator $g$, pretrained TransE as the discriminator $d$,
    a mini-batch of data $X_b$, the number of samples for ranking, $K$,
    learning rate $\eta$, weight decay rate $\gamma$.}
    \KwResult{Adversarially trained generator $g$ and $d$ models}
    \SetKwFunction{GetRank}{GetRank}%
    \SetKwFunction{softmax}{SoftMax}%
    \SetKwFunction{gumbelsoft}{GumbelSoftmax}%
    \Begin{
        $grad_d \longleftarrow 0, grad_g \longleftarrow 0$ \tcp*{empty the gradients}
        \tcp{Train the discriminator, i.e., TransE}
        \For{$(h, r, t)\in X_b$}{
            $logits \longleftarrow g(h, r)$ \;
            $distribution \longleftarrow $ \softmax{$logits$} \;
            $\hat t_- \longleftarrow$ draw 1 sample from the predicted $distribution$ of $t$ \;
            $\hat t_+ \longleftarrow$ draw $K$ samples from the predicted $distribution$ \;
            
            $loss_d \longleftarrow $ \GetRank{$t$, $\hat t_+$} $-$ \GetRank{$\hat t_-$, $t$} \tcp*{compute rankings as (\ref{eq:ranking}) to build loss}
    
            $grad_d \longleftarrow -\bigtriangledown loss_d$
            \tcp*{add up the gradient for this example}
        }
        $\Theta_d \longleftarrow \Theta_d - \eta grad_d - \gamma \Theta_d $ \tcp*{update the discriminator model}
        \tcp{Train the generator}
        \For{$(h, r, t)\in X_b$}{
            $logits \longleftarrow g(h, r)$ \;
            $sample \longleftarrow $ \gumbelsoft{$logits$} \tcp*{follow (\ref{eq:gumbel-softmax}) to draw a sample from the output multinomial distribution}
            \tcp{use discriminator scores to build up the loss function of generator }
            $score_{t'} \leftarrow d(h, r, sample), score_t \leftarrow d(h, r, t)$ \;
            $loss_g \longleftarrow - \exp(score_{t'}) / (\exp(score_{t'}) + \exp(score_{t}))$ \;
            $grad_g \longleftarrow \bigtriangledown loss_g$ \tcp*{add up the gradient for the generator}
        }
        $\Theta_g \longleftarrow \Theta_g - \eta grad_g - \gamma \Theta_g$
        \tcp*{update the generator parameters}
    }
    \caption{Adversarial Training in each iteration\label{algo:adv-training}}
\end{algorithm*}
...
\begin{table}[htb]
    ...
    \parbox[b]{\columnwidth}{
        ...
        \begin{tabular}{c|r|r|r|r}
            ...
            \bottomrule
        \end{tabular}
        \subcaption{Results of tail prediction}        
    }
    ...
    \parbox[b]{\columnwidth}{
        ...
        \begin{tabular}{c|r|r|r|r}
            ...
            \bottomrule
        \end{tabular}
        \subcaption{Results of head prediction}        
    }
